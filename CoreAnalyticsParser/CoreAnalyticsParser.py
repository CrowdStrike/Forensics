...
def CoreAnalyticsParser():
	...
	mutu.add_argument('-i','--input',default='./',help='input file or directory containing core_analytics files and/or aggregate files',required=False)
	mutu.add_argument('-d','--disk',default=False,help='parse all core_analytics files and aggregates on current disk',required=False, action='store_true')
	aparser.add_argument('-o','--outputdir',default='./',help='output directory',required=False)
	aparser.add_argument('-j','--json',default=False,help='produce output in JSON format',required=False, action='store_true')
	args = aparser.parse_args()

	outfile_loc = (args.outputdir+'/CoreAnalyticsParser_output').replace('//','/').replace('//','/')

	headers = ['src_report', 'diag_start', 'diag_end','name','uuid','processName','appDescription','appName',
	'appVersion','foreground','uptime','uptime_parsed','powerTime','powerTime_parsed','activeTime','activeTime_parsed',
	'activations','launches','activityPeriods','idleTimeouts','Uptime','Count','version','identifier','overflow']
	...
	if os.geteuid() != 0:
		print "[!] You are not running this script as sudo, but you are trying to parse files on the local disk. Parsing of aggregate files may fail!"
	...
	for file in analytics_location:
		...
		try:
			diag_start = [json.loads(i) for i in data.split('\n') if i.startswith("{\"_marker\":") and "end-of-file" not in i][0]['startTimestamp']
		except:
			diag_start = "ERROR"

		try:
			diag_end = [json.loads(i) for i in data.split('\n') if i.startswith("{\"timestamp\":")][0]['timestamp']
			diag_end = str(parser.parse(diag_end).astimezone(pytz.utc)).replace(' ','T').replace('+00:00','Z')
		except:
			diag_end = "ERROR" 
		...
		for i in data_lines:
			...
			record['name'] = i['name']
			record['uuid'] = i['uuid']
			record['overflow'] = {} # If any fields not currently recorded (based on the headers above) appear, they will be added to overflow.
			...
			if record['uptime'] != '':
				record['uptime_parsed'] = time.strftime("%H:%M:%S", time.gmtime(record['uptime']))

			if record['activeTime'] != '':
				record['activeTime_parsed'] = time.strftime("%H:%M:%S", time.gmtime(record['activeTime']))

			if record['powerTime'] != '':
				record['powerTime_parsed'] = time.strftime("%H:%M:%S", time.gmtime(record['powerTime']))
